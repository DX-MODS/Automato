...
from helper.database.afkdb import get_afk_status
from helper.database.pmpermitdb import get_approved_users, pm_guard
import helper.database.welcomedb as Automatodb
import shlex
...
async def welcome_chat(filter, client: Client, message: Message):
    to_welcome = await Automatodb.get_welcome(str(message.chat.id))
    if to_welcome:
        return True
    else:
        return False
